...
<p style="color:#9BCD9B;font-size:20px;">
  1. 正常情况,加载 script 标签，是同步的但是 script 标签，会阻塞加载，放在html页面的head标签，肯定会影响用户体验。加载页面时候，会出现一段时间的空白，当然，可以：
    1> 加一张 gif 或 动画 ，作为等待框，直至加载完毕，再隐藏等待框。
    2> 将script标签放到body的底部，不会影响dom和style的加载，提升用户体验。
    3> 异步加载 script 在 script标签加上defer属性，之后就是无阻塞请求文件。但是自己的业务逻辑的js代码，会依赖js库，就会报错，所以就要在script onload事件之后，执行自己的业务代码。
</p>
